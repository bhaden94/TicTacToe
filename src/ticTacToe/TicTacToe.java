/**
 * TicTacToe.java Brady Haden
 * 
 * This GUI program displays a tic-tac-toe board and allows two players to try
 * and best one another in the classic game. When a player wins or there is a
 * tie the board is automatically reset for another round.
 * 
 */
...
public class TicTacToe extends JFrame
{
  ...
  public TicTacToe()
  {
    ...
    createContents();
    // matches the users system styling
    try
    {
  ...
  /*
   * This creates a grid layout of buttons using the 2s array buttonsArr
   */
      ...
      if (btn.getText().isEmpty())
      {
        ...
        if (checkForWinner()) // true if current xTurn is winner
        {
          JOptionPane.showMessageDialog(null,
              (xTurn ? "X" : "O") + " is the winner!");
          resetGame();
        }
        // if board is full with no winner then tie game
  ...
  /*
   * method to reset the game. it clears the board and sets xTurn to its default
   * value of 'true'.
   */
  ...
  /*
   * this checks for a winner by checking if each row on the board has 3 in a
   * row or each column nhas 3 in a row. it then checks both the major
   * diagonal(top left to bottom right) and the minor diagonal(top right to
   * bottom left) for 3 in a row
   * 
   * returns boolean
   */
